...
parser.add_argument('--qual_results', action='store_true', help='uses reproducable dataset for rendering of qualitative results')
...
parser.add_argument("--max_neurons_merged", type=int, default=4, help='max neurons merged into a single point cloud')
...
def main():
    ...
    if args.qual_results:
        reproducible_path = os.path.join(os.path.dirname(args.types_path), 'reproducable_items')
        dataset_test = build_reproducible_dataset(
            path=reproducible_path
        )
    ...
    for t in thresholds:
        with torch.no_grad():
            ...
            for i, (pc, labels, mask, root_ids, queries, pairs, pairs_labels)  in enumerate(tqdm(data_loader_test)):
                ...
                # mask pred affinity
                pred_aff_masked = pred_affinity[..., :masked_pc.shape[0], :masked_pc.shape[0]]
                ...
                mapped_labels = misc.get_mapped_labels(gt_labels=masked_labels, labels=pred_labels) # make sure color assignment is consistent

                if args.store_tensors:
                    images = torch.tensor(pred_aff_masked, dtype=torch.float32)
                    misc.save_images(args.output_dir, images, root_ids, folder='affinity')
                    misc.save_points(args.output_dir, pc, root_ids, folder='points')
                    misc.save_points(args.output_dir, mapped_labels.unsqueeze(0), root_ids, folder='points', suffix='_labels')
                    misc.qual_plot(args.output_dir, pc[mask].squeeze(), mapped_labels.squeeze(), root_ids.squeeze(), suffix='_ours', folder='qualitative')
                    misc.qual_plot(args.output_dir, pc[mask].squeeze(), masked_labels.squeeze(), root_ids.squeeze(), suffix='_gt', folder='qualitative')
                    misc.qual_plot(args.output_dir, pc[mask].squeeze(), torch.zeros_like(masked_labels.squeeze()), root_ids.squeeze(), suffix='_input', folder='qualitative')
        ...
        string_buffer.write(f"-----------------------------\n")
        string_buffer.write(f"Variation of Information: {torch.round(voi_mean, decimals=2)}\n")
        string_buffer.write(f"Variation of Information Split: {torch.round(voi_split_mean, decimals=2)}\n")
        string_buffer.write(f"Variation of Information Merge: {torch.round(voi_merge_mean, decimals=2)}\n")
        string_buffer.write(f"-----------------------------\n")
        string_buffer.write(f"Adjusted Rand Error: {torch.round(are_mean, decimals=2)}\n")
        string_buffer.write(f"Adjusted Rand Error Precision: {torch.round(are_prec_mean, decimals=2)}\n")
        string_buffer.write(f"Adjusted Rand Error Recall: {torch.round(are_recall_mean, decimals=2)}\n")
        string_buffer.write(f"-----------------------------\n")
        string_buffer.write(f"Adjusted Rand Score: {torch.round(ars_mean, decimals=2)}\n")
        string_buffer.write(f"-----------------------------")
        string_buffer.write(f"\n")
